...
INITIAL_POSE_SIGMA = np.concatenate((np.deg2rad([1, 1, 1]), [0.1, 0.01, 1]))
...
class BundleAdjusment:
    ...
    def create_and_solve_all_bundle_windows(self):
        for i in tqdm(range(len(self.key_frames) - 1)):
            ...
            self.key_frames_to_landmarks_relative_coordinates[window_start_frame] = all_landmarks
        self._is_optimized = True
    ...
    def choose_key_frames_by_median_track_len(self):
        ...
        while key_frames[-1] < n - 1:
            ...
            tracks = np.array(self._tracking_db.frameId_to_trackIds_list[frame])
            ...
            new_key_frame = tracks_lens[int(len(tracks_lens) / 1.2)] + last_key_frame
