...
def get_y_distance_between_points(matches, key_points_1, key_points_2):
    ...
    for match in matches:
        x1, x2, y1, y2 = utils.get_pixels_from_matches(key_points_1, key_points_2, match)
        y_dist = np.abs(y1 - y2)
...
def check_times():
    ...
    P1, P2 = k @ m1, k @ m2
    key_points_1, key_points_2, matches = utils.match_pair(0)
    good_matches_inds = reject_outliers_using_y_dist(key_points_1, key_points_2, matches, Y_DISTANCE_THRESHOLD)
...
def Q22():
    ...
    utils.display(left_img, right_img, key_points_1, key_points_2, good_matches, bad_matches,
...
def Q24():
    ...
    k, m1, m2 = utils.read_cameras()
    P1, P2 = k @ m1, k @ m2
    for image_pair in range(5, 10):
        key_points_1, key_points_2, matches = utils.match_pair(image_pair)
        good_matches_inds = reject_outliers_using_y_dist(key_points_1, key_points_2, matches, Y_DISTANCE_THRESHOLD)
        good_matches = [match for match, is_good in zip(matches, good_matches_inds) if is_good]
        points_a, points_b = get_points_from_matches(good_matches, key_points_1, key_points_2)
        x_3d_mine = utils.triangulate_points(P1, P2, points_a, points_b)
