...
def fill_tracking_db():
    ...
    tracking_db = initialize_tracking_db()
    ...
    for frame_idx in range(num_frames):
        ...
        cur_frameId = tracking_db.add_frame(links=links, left_features=features_left, right_features=features_right,
...
def fill_tracking_db3():
    ...
    for frame_idx in range(num_frames):
        print(frame_idx)
        left, right = utils.read_images(frame_idx)
        cur_left_descriptors, cur_right_descriptors, left_key_points, matches_dict, right_key_points, matches_init = processor.get_matches(
            left, right)
        ...
        # find outliers using RANSAC now
        prev_left_descriptors = tracking_db.last_features()
        prev_links = tracking_db.prev_frame_links
        prev_matches_dict = create_trivial_dict(len(prev_links))
        left_0_left_1_matches = utils.find_closest_features(prev_left_descriptors, cur_left_descriptors)
        ...
        left_0_points_2D = []
        right_0_points_2D = []
        for lnk in prev_links:
            x_left = lnk.x_left
            x_right = lnk.x_right
            y = lnk.y
            left_0_points_2D.append([x_left, y])
            right_0_points_2D.append([x_right, y])

        left_0_points_2D = np.array(left_0_points_2D)[left_0_inds]
        right_0_points_2D = np.array(right_0_points_2D)[right_0_inds]

        left_1_points_2D = [left_key_points[i] for i in left_1_inds]
        right_1_points_2D = [right_key_points[i] for i in right_1_inds]
        left_1_points_2D, right_1_points_2D = utils.get_points_from_key_points(left_1_points_2D,
                                                                               right_1_points_2D)
        ...
        for i in range(len(features_left)):
            desc0 = features_left[i]
            desc1 = features_right[i]
            distance = np.linalg.norm(desc1 - desc0)
            match = cv2.DMatch(_queryIdx=i, _trainIdx=i, _imgIdx=0, _distance=distance)
            matches_right_left.append(match)
        _, _, links = tracking_db.create_links(left_features=features_left,
                                               right_features=features_right,
                                                   kp_left=kp_left,
                                                   kp_right=kp_right,
        ...
        prev_features = tracking_db.last_features()
        prev_features_inliers = prev_features[left_0_inliers]
        # matches_to_previous_left = utils.find_closest_features(prev_features, features_left)
        matches_to_previous_left = []
        num_matches = len(left_0_inliers)
        for i in range(num_matches):
            desc0 = prev_features_inliers[i]
            desc1 = features_left[i]
            distance = np.linalg.norm(desc1 - desc0)
            match = cv2.DMatch(_queryIdx=left_0_inliers[i], _trainIdx=i, _imgIdx=0, _distance=distance)
            matches_to_previous_left.append(match)
